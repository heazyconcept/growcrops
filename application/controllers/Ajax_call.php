...
class Ajax_call extends CI_Controller
{
    ...
    public function transaction()
    {
        ...
        if ($payment_type == "bank_transfer") {
            $insertData = array(
                "UserId" => (isset($user_id)) ? $user_id : $this->session->userdata("user_id"),
        ...
        if ($response > 0) {
            ...
            $r = $this->notificationmail->send_mail(json_encode($mailOptions));
            log_message('error', "GCO-Email: " . $r);
            ...
            if ($payment_type == "Paystack") {
                ...
                $rs = $this->receiptmail->send_mail(json_encode($mailOptions));
                log_message('error', "GCO-Email: " . $rs);
            }
            echo 1;
