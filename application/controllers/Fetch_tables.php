...
class Fetch_tables extends CI_Controller
{
    ...
    public function pending_transaction()
    {
        ...
        } // }elseif ($this->input->get('dateSearch')) {
        //     # code...
        // }
    ...
    public function transactions()
    {
        $this->load->model('pagination_model');
        // echo "<pre>";
        // print_r($this->input->get('dateSearch'));
        // echo "</pre>";
        // die();
        ...
        if (empty($this->input->get('search')['value'])) {
            if ($this->input->get('dateSearch') == 1) {
    ...
    public function employees()
    {
        ...
        $dbOption = array(
            "table_name" => "users",
            "targets" => array("user_role" => 2, "designation" => 'employee'),
            "operator" => "AND"
        );
        $totalData = $this->connectDb->count_data(json_encode($dbOption));
        $totalFiltered = $totalData;
        if (empty($this->input->get('search')['value'])) {
            ...
            $users = $this->connectDb->select_data(json_encode($dbOption));

        } else {
            $search = $this->input->get('search')['value'];
            $query = "SELECT * FROM users WHERE
            user_role = 2 AND designaion = 'employee' AND first_name like '%$search%' OR
            user_role = 2 AND designaion = 'employee' AND last_name like '%$search%' OR
            user_role = 2 AND designaion = 'employee' AND email_address like '%$search%' OR
            ";
            $dbOption = array(
                "table_name" => "users",
                "my_query" => $query,
        ...
        if (!empty($users)) {
            foreach ($users as $obj) {
                ...
                $nestedData['email_address'] = $obj->email_address;
                $nestedData['phone_number'] = $obj->phone_number;
                $nestedData['action'] = '<a class="btn btn-danger" href="'.base_url('admin/deleteuser/'. $obj->id).'">DELETE</a>';
                $data[] = $nestedData;
